...
def FA(x,y,cin):
    ...
    return (gor(gand(XOR(x,y),cin),gand(x,y)),XOR(cin,XOR(x,y)))
...
def fourBitAdd(xxxx,yyyy):
    ...
    and c is the carry.'''
    (carry, zo) = FA(xxxx[3],yyyy[3],0)
    (carry, zt) = FA(xxxx[2],yyyy[2],carry)
    (carry, zf) = FA(xxxx[1],yyyy[1],carry)
    (carry, ze) = FA(xxxx[0],yyyy[0],carry)
    return (carry, (ze,zf,zt,zo))
